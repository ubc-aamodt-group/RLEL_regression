...
class LearnableTransform(nn.Module):
    def __init__(self, vrange, bits,init,code):
        super(LearnableTransform, self).__init__()
        self.params = torch.nn.Parameter(torch.rand(size=(bits, vrange), dtype=torch.float, requires_grad=True))
        if init=="code":
            print(code)
            with torch.no_grad():
                self.params.copy_(torch.transpose(torch.tensor(code),0,1))
        self.bits = bits
        self.val_range = vrange


    def forward(self, x):
        a = torch.zeros((self.bits), dtype=torch.float32)
        #print(a.size())
        #print(self.params.size(),x)
        a = self.params.clone().detach().cpu()[torch.tensor(x, dtype=torch.long)]
        return a
    ...
    def convert_discrete(self, x):
        ...
        for i in range(N):
            ...
            # print(match_decode(x[i, :], self.bits, self.di))
            # exit()
            y = match(x[i, :].unsqueeze(0), self.val_range, self.params.cuda())
            a[i] = y
    ...
    def convert_continuous(self, x):
        ...
        for i in range(N):
            ...
            # print(match_decode(x[i, :], self.bits, self.di))
            # exit()
            y = match_decode(x[i, :].unsqueeze(0), self.val_range, self.params.cuda())
            a[i] = y
        return a
    ...
    def get_cor(self, x):
        ...
        a = torch.zeros((N, self.val_range), dtype=torch.float32).cuda()
        for i in range(N):
            y = torch.matmul(x[i,:], self.params.cuda())
            a[i] = y
        return a
