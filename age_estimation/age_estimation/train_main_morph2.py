...
TRAIN_CSV_PATH = '../dataset/morph2_train.csv'
VALID_CSV_PATH = '../dataset/morph2_valid.csv'
TEST_CSV_PATH = '../dataset/morph2_test.csv'
IMAGE_PATH = '../dataset/morph2'
...
def train_loop(args, num_classes, result_transform, train_loader, valid_loader, test_loader, criterion):
    baseLabel = args.dataset + "_" + args.transform + "_" + args.reverse_transform + "_" + args.loss + str(args.const_weight).replace(".","o") + str(args.dist_weight).replace(".","o")+ args.version
    ...
    if args.model == 'resnet':
        model = get_age_resnet(num_classes,62, 50, True, 128,args.init)
    ...
    for name,param in model.named_parameters():
        ...
        if not("code" in name):
           params.append(param)
    for param in result_transform.parameters():
        codeparams.append(param)
    mu=0.0
    if args.reqgrad>0.01:
        mu = 10.0
    #print(model)
    #print(model.code)
    optimizer = torch.optim.Adam([{'params':params, 'lr':0.0001},{'params':codeparams, 'lr':0.0001*mu},{'params':[model.code], 'lr':0.0001*mu}]) 
