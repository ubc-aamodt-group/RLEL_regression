...
def parse_args():
    ...
    parser.add_argument('--init', dest='init', help='p0 or p1 or rand or code_specific', default='rand', type=str)
    parser.add_argument('--norm_weight', dest='norm_weight', help='weight to constraint loss.',
          default=0.0, type=float)
    parser.add_argument('--dist_weight', dest='dist_weight', help='weight to constraint loss.',
          default=0.0, type=float)
    parser.add_argument('--const_weight', dest='const_weight', help='weight to constraint loss.',
          default=0.0, type=float)
    parser.add_argument('--reqgrad', dest='reqgrad', help='p0 or p1 or rand or code_specific', default=0.0, type=float)  
...
if __name__ == '__main__':
    ...
    # ResNet50
    model = hopenet.ResNet(torchvision.models.resnet.Bottleneck, [3, 4, 6, 3], 3,num_bits=args.num_bits, code=args.code,init=args.init,reqgrad=args.reqgrad)
    ...
    helper.train(model,train_loader, test_loader, pose_dataset, args.output_string, gpu, args.arch,args.lr,20,batch_size,195,args.num_bits,args.code_bits,args.code,loss_func,di,dis,args,args.reqgrad)
