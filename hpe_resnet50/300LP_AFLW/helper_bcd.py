...
def measure_MAE(cont_labels,yaw,pitch,roll,gpu,func,yaw_code,pitch_code,roll_code):
    ...
    yaw_predicted=func(yaw,200,yaw_code).cpu()
    pitch_predicted= func(pitch,200,pitch_code).cpu()
    roll_predicted=func(roll,200,roll_code).cpu()
...
def train(model,train_loader,test_loader, pose_dataset, output_string, gpu, arch,lr,num_epochs,batch_size,val_bound,num_bits,code_bits,code,loss_func,di,dis,args,reqgrad):
    ...
    t150_array = torch.tensor(range(0,200,1)).cuda()
    t100_array = torch.tensor(range(0,200,1)).cuda()
    #tensors for the correlation
    best_MAE=100
    weight_c = torch.tensor([[[[-1.0,1.0]]]])
    epoch_list=[1,10,18,19,20,25,35,num_epochs-2,num_epochs-1]
