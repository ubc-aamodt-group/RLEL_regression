...
class DrivingDatasetDataset(Dataset):
    def __init__(self, cfg, data_dir, ann_path, transform):
        self.cfg = cfg
    ...
    def __getitem__(self, idx):
        ...
        steering_command = torch.tensor(steering_command + self.offset)
        steering_command_level = torch.tensor(steering_command, dtype=int)
        steering_level = self.transform(steering_command_level)
        steering_level = (steering_level - 0.5).sign()
        steering_level = torch.tensor(steering_level, dtype=torch.float32)
        return image, steering_command, steering_level
